...
func (f *Flow) PopulateFrom(val *dynamic.Message) {
	...
	for _, ifield := range fields.([]interface{}) {
		...
		case 6: // VLAN_ID
			...
			fmt.Println(vid, mask, mask.(uint32), errMaskGet)
			fmt.Println(vid == 0,  errMaskGet == nil, mask.(uint32) == 4096)
			if vid == 0 && errMaskGet == nil && mask.(uint32) == 4096 {
				fmt.Println("setting ANY")
				f.VlanId = "any"
			} else {
