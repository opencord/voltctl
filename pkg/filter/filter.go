...
func (f Filter) EvaluateTerm(k string, v FilterTerm, val reflect.Value, recurse bool) (bool, error) {
	...
	if strings.Contains(k, ".") {
		...
		if val.Kind() != reflect.Struct {
			return false, fmt.Errorf("Value %v is not a struct while filtering\n", val)
		}
		field := val.FieldByName(parts[0])
		if !field.IsValid() {
			return false, fmt.Errorf("Failed to find dotted field %s while filtering\n", parts[0])
		}
	...
	if !field.IsValid() {
		return false, fmt.Errorf("Failed to find field %s while filtering\n", k)
	}
