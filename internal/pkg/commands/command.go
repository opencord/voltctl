...
func ReadConfig() {
	...
	if info, err := os.Stat(GlobalOptions.Config); err == nil && !info.IsDir() {
		...
		// If the config file was specified, then the user expects
		// the file to be there, so error out if it is not
		if specified {
			Error.Fatalf("Unable to read specified config file '%s' : %s",
				GlobalOptions.Config, err.Error())
		}
	}
}
