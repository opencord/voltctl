...
const (
  ...
  CONNECTSTATUS: {{.ConnectStatus}}`
	...
	ONU_IMAGE_LIST_FORMAT			    = "table{{.Version}}\t{{.Commited}}\t{{.Active}}\t{{.Valid}}\t{{.ProductCode}}"
...
type OnuDownloadImage struct {
	Args struct {
		...
		Url               string   `positional-arg-name:"IMAGE_URL" required:"yes"`
		ImageVersion      string   `positional-arg-name:"IMAGE_VERSION" required:"yes"`
		vendor            string   `positional-arg-name:"IMAGE_VENDOR" required:"yes"`
		ActivateOnSuccess bool   `positional-arg-name:"IMAGE_ACTIVATE_ON_SUCCESS" required:"yes"`
		CommitOnSuccess   bool   `positional-arg-name:"IMAGE_COMMIT_ON_SUCCESS" required:"yes"`
		Crc               uint32   `positional-arg-name:"IMAGE_CRC" required:"yes"`
	} `positional-args:"yes"`
}
...
func (options *OnuDownloadImage) Execute(args []string) error {
	...
	var devIDList = []*common.ID {
		&common.ID{Id: string(options.Args.Id)},
	}
