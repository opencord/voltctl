...
PmFormat:	                {{.PmFormat}}`
	DEFAULT_ONU_OMCI_TX_RX_STATS_FORMAT = `BASE_TX_AR_FRAMES: {{.BaseTxArFrames}}
BASE_RX_AK_FRAMES: {{.BaseRxAkFrames}}
BASE_TX_NO_AR_FRAMES: {{.BaseTxNoArFrames}}
BASE_RX_NO_AK_FRAMES: {{.BaseRxNoAkFrames}}
EXTENDED_TX_AR_FRAMES: {{.ExtTxArFrames}}
EXTENDED_RX_AK_FRAMES: {{.ExtRxAkFrames}}
EXTENDED_TX_NO_AR_FRAMES: {{.ExtTxNoArFrames}}
EXTENDED_RX_NO_AK_FRAMES: {{.ExtRxNoAkFrames}}
TX_OMCI_COUNTER_RETRIES: {{.TxOmciCounterRetries}}
TX_OMCI_COUNTER_TIMEOUTS: {{.TxOmciCounterTimeouts}}`
)
...
func (options *OnuOmciTxRxStats) Execute(args []string) error {
	...
	//data := buildOnuEthernetFrameExtendedPmOutputFormat(rv.GetResponse().GetOnuCounters())
	//if outputFormat == "" {
	//	outputFormat = GetCommandOptionWithDefault("device-get-onu-status", "format", DEFAULT_ONU_OMCI_TX_RX_STATS_FORMAT)
	//}
	result := CommandResult{
		Format:    format.Format(outputFormat),
		OutputAs:  toOutputType(options.OutputAs),
