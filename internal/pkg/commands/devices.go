	...
	if pmConfigs.Grouped == false {
		for _, metric := range pmConfigs.Metrics {
			for _, mName := range options.Args.Metrics {
				if string(mName) == metric.Name && metric.Enabled == false {
					metric.Enabled = true
					client.UpdateDevicePmConfigs(ctx, pmConfigs)
				}
			}
		}
	}
...
func (options *DevicePmConfigMetricDisable) Execute(args []string) error {
	...
	if pmConfigs.Grouped == false {
		for _, metric := range pmConfigs.Metrics {
			for _, mName := range options.Args.Metrics {
				if string(mName) == metric.Name && metric.Enabled == true {
	...
	if pmConfigs.Grouped {
		for _, group := range pmConfigs.Groups {
			for _, gName := range options.Args.Groups {
				if string(gName) == group.GroupName && group.Enabled == false {
					group.Enabled = true
					client.UpdateDevicePmConfigs(ctx, pmConfigs)
				}
			}
...
func (options *DevicePmConfigGroupDisable) Execute(args []string) error {
	...
	if pmConfigs.Grouped {
		for _, group := range pmConfigs.Groups {
			for _, gName := range options.Args.Groups {
				if string(gName) == group.GroupName && group.Enabled == true {
					group.Enabled = false
					client.UpdateDevicePmConfigs(ctx, pmConfigs)
				}
			}
