...
const (
	DEFAULT_EVENT_FORMAT = "table{{.Category}}\t{{.SubCategory}}\t{{.EvType}}"
)
...
func PrintHeader(outputFormat string, outputAs string, hdr *EventHeader) error {
...
func (options *EventListenOpts) Execute(args []string) error {
	...
	if options.ShowBody {
		...
		header, err := FixedWidthFormatter(DEFAULT_EVENT_FORMAT, DefaultWidths, true, nil)
		if err != nil {
			return err
		}
		fmt.Println(header)
		...
		for {
			...
			case msg := <-consumer:
				...
				if options.ShowBody {
					...
					err := PrintHeader(options.Format, DEFAULT_EVENT_FORMAT, hdr)
					if err != nil {
						log.Printf("%v\n", err)
					}
				}
