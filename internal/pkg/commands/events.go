...
func PrintHeader(format string, hdr *EventHeader) {
	if format == "csv" {
		// comma-separated value format imported into excel
		fmt.Printf("%s,%s,%s,%f,%f,%s,%s,%d,%d\n",
			hdr.Category,
			hdr.SubCategory,
			hdr.EvType,
			hdr.Raised_ts,
			hdr.Reported_ts,
			"\""+strings.Join(hdr.Titles, ",")+"\"",
			"\""+strings.Join(hdr.Device_ids, ",")+"\"",
			hdr.Timestamp.Unix(),
			hdr.Timestamp.Minute())
	} else if format == "json" {
		// only prints {}. weird.
		...
		// prints nasty golang struct format. Not useful.
		fmt.Printf("%v\n", hdr)
	}
}
