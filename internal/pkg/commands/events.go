	...
	Loop:
		for {
			...
			case consumerError := <-consumerErrors:
				msgCount++
				log.Printf("Received consumerError topic=%v, partition=%v, err=%v", string(consumerError.Topic), string(consumerError.Partition), consumerError.Err)
				doneCh <- struct{}{}
			case <-signals:
				doneCh <- struct{}{}
			}
...
func consume(topics []string, master sarama.Consumer) (chan *sarama.ConsumerMessage, chan *sarama.ConsumerError) {
	...
	for _, topic := range topics {
		...
		if nil != err {
			log.Printf("Error in consum(): Topic %v Partitions: %v", topic, partitions)
			panic(err)
		}
