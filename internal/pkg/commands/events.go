...
func (options *EventListenOpts) StartOutput() error {
	if options.OutputAs == "json" {
		fmt.Println("[")
	} else if (options.OutputAs == "table") && !options.ShowBody {
		f := format.Format(DEFAULT_EVENT_FORMAT)
		...
		for {
			...
			case msg := <-consumer:
				...
				if options.ShowBody {
					...
					err := PrintEventHeader(options.OutputAs, DEFAULT_EVENT_FORMAT, hdr)
					if err != nil {
						log.Printf("%v\n", err)
					}
				}
