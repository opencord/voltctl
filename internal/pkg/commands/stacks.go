...
type StackOptions struct {
	List   StackList   `command:"list"`
	Add    StackAdd    `command:"add"`
	Delete StackDelete `command:"delete"`
	Use    StackUse    `command:"use"`
}
...
func (options *StackAdd) Execute(args []string) error {
	...
	if GlobalConfig.GetCurrentStack() == "" {
		GlobalConfig.SetCurrentStack(options.Args.Name)
	}
	if err := GlobalConfig.Write(GlobalOptions.Config); err != nil {
		Error.Fatal(err.Error())
	}
