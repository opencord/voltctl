...
func (options *SetLogLevelOpts) Execute(args []string) error {
	...
	client, err := kvstore.NewEtcdClient(GlobalConfig.KvStoreConfig.KVStoreHost+":"+strconv.Itoa(GlobalConfig.KvStoreConfig.KVStorePort), GlobalConfig.KvStoreConfig.KVStoreTimeout)
	if err != nil {
		return fmt.Errorf("Unable to create kvstore client %s", err)
	}
	defer client.Close()

	cm := config.NewConfigManager(client, GlobalConfig.KvStoreConfig.KVStoreType, GlobalConfig.KvStoreConfig.KVStoreHost, GlobalConfig.KvStoreConfig.KVStorePort, GlobalConfig.KvStoreConfig.KVStoreTimeout)

	var output []LogLevelOutput

	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(GlobalConfig.KvStoreConfig.KVStoreTimeout)*time.Second)
	defer cancel()
