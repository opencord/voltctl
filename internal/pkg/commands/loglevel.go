...
func BuildKubernetesNameMap() (map[string]string, map[string]string, error) {
	...
	for _, pod := range pods.Items {
		...
		case "voltha-api-server":
			kube_to_arouter[pod.Name] = "api-server"
		case "rw-core":
			...
			kube_to_arouter[pod.Name] = "vcore" + affinity_group + ".vcore" + affinity_group + affinity_group_core_id
		case "ro-core":
			kube_to_arouter[pod.Name] = "ro_vcore0.ro_vcore01"
		}
	}

	for k, v := range kube_to_arouter {
		arouter_to_kube[v] = k
	}
...
func ValidateComponentName(s string) error {
	...
	case "ro_vcore0":
		return nil
	case "ro_vcore0.ro_vcore01":
		return nil

	// rwcore currently unsupported
...
func (options *SetLogLevelOpts) Execute(args []string) error {
	...
	for _, kubeComponentName := range options.Args.Component {
		...
		componentName, ok := kube_to_arouter[kubeComponentName]
		if !ok {
			return fmt.Errorf("Unknown component %s", kubeComponentName)
		}

		if componentName == "api-server" {
			conn, err = NewApiServerConnection()
			if err != nil {
				return err
			}
			defer conn.Close()
		...
		*/
		...
		enumLogLevel := logLevelEnumType.FindValueByName(options.Args.Level)
		...
		ll["level"] = enumLogLevel.GetNumber()
