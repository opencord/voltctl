...
const (
	...
	DEFAULT_SETLOGLEVEL_FORMAT = "table{{ .ComponentName }}\t{{.Status}}"
)
	...
	for _, componentName := range options.Args.Component {
		...
		*/
		...
		if success.(bool) {
			...
			output = append(output, SetLogLevelOutput{ComponentName: componentName, Status: "Failure", Error: errorText.(string)})
		}
	}
