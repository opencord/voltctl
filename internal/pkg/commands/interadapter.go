...
func ExtractJSON(m interface{}, fieldName string, def interface{}) interface{} {
	parts := strings.Split(fieldName, ".")
	for _, s := range parts {
		var okay bool
		m, okay = m.(map[string]interface{})[s]
		if !okay {
			return def
		}
	}
...
func ExtractJSON_string(m interface{}, fieldName string, def string) (string, error) {
	intf := ExtractJSON(m, fieldName, def)
	s, okay := intf.(string)
	if !okay {
		return "", fmt.Errorf("Failed to convert string %v", s)
	}
	return s, nil
}

func ExtractJSON_int64(m interface{}, fieldName string, def int64) (int64, error) {
	s, err := ExtractJSON_string(m, fieldName, strconv.FormatInt(def, 10))
	if err != nil {
		return 0, err
	}
...
func DecodeInterContainerHeader(md *desc.MessageDescriptor, b []byte, ts time.Time, f grpcurl.Formatter) (*InterContainerHeader, error) {
	// this attempt got really ugly really fast...
	//    will abandon.
	...
	id := ExtractJSON(js, "header.id", "").(string)
	msgType := ExtractJSON(js, "header.type", "").(string)
	fromTopic := ExtractJSON(js, "header.fromTopic", "").(string)
	toTopic := ExtractJSON(js, "header.toTopic", "").(string)
	keyTopic := ExtractJSON(js, "header.keyTopic", "").(string)
	timestampInt64, err := ExtractJSON_int64(js, "header.timestamp", 0)
	if err != nil {
		return nil, err
	}

	timestamp, err := DecodeTimestamp(timestampInt64)
	if err != nil {
		return nil, err
	}

	iaHeader := InterContainerHeader{Id: id,
		Type:      msgType, //model.GetEnumString(header, "type", msgType),
		FromTopic: fromTopic,
		ToTopic:   toTopic,
		KeyTopic:  keyTopic,
		Timestamp: timestamp}
