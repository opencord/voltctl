...
func toStringArray(cnl []ComponentName) []string {
	var list []string
	for _, cn := range cnl {
...
func getPackageNames(component string) ([]string, error) {
...
func (options *ListLogPackagesOpts) Execute(args []string) error {
	...
	orderBy := options.OrderBy
	if orderBy == "" {
		orderBy = GetCommandOptionWithDefault("log-package-list", "order", "PackageName,ComponentName,Level")
	}
